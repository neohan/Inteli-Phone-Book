...
namespace InteliPhoneBookService
{
    class FSXMLCurlProcessor
    {
        ...
        public class ModCurlHandler
        {
            ...
            public ModCurlHandler(FSXMLCurlProcessor p_parent)
            {
                server = new HttpWebServer(p_parent.CurlPort);
        ...
        public void Initialize()
        {
            ...
            {
                try { CurlPort = Int32.Parse(ConfigurationManager.AppSettings["CurlPort"]); }
                catch (Exception e) { CurlPort = 7717; }
            }
        }

        static public void DoWork(Object stateInfo)
        {
            FSXMLCurlProcessor curlProcessor = (FSXMLCurlProcessor)stateInfo;
            curlProcessor.Initialize();
            while (true)
            {
